...
public class Collision {
	...
	private final double LENIENCY_CONSTANT = 1.0;
	...
	// Line extrapolation to get intermediate points
	...
	// Distance calculation to find shortest distance between line segment and point
    ...
    public boolean checkCollisionsVectors(Point a, Point b, double xc, double yc, double circ_rad){
        if (a == null || b == null || b.getCollisionFlag() || ((a.getX() == b.getX()) && (a.getY() == b.getY()))){
        	return false; //invalid points
        }
    	Point closest = closest_point_on_seg(a, b, xc, yc);
        Point circ_pos = new Point(xc, yc);
        closeness = circ_pos.distance(closest); // accuracy: distance from center vs circle radius
        if (closeness > LENIENCY_CONSTANT * circ_rad){ 
